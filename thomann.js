...
async function scrape() {
  ...
  await log(SUPPLIER, "Start van thomann.js", "start");
  ...
  for (let pageNum = startPage; totalPages === null || pageNum <= totalPages; pageNum++) {
    ...
    try {
      ...
      await log(SUPPLIER, `Waarschuwing: geen productkaarten gevonden op pagina ${pageNum} (${url}), scrape gestopt.`, "warning");
      break;
    }
  ...
  await log(SUPPLIER, `Einde van thomann.js: ${totalSaved} opgeslagen (${totalFound} gevonden)`, "success");
...
scrape().catch(async (err) => {
  console.error(err);
  await log(SUPPLIER, `Fout in thomann.js: ${err.message}`, "error");
  process.exit(1);
});
